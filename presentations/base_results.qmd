...
::: {layout-ncol=2}
![](../models/2023/DiscardVariance/bridging/1-compare1_spawnbio.png)

![](../models/2023/DiscardVariance/bridging/2-compare1_spawnbio.png)
:::

## Base model

::: {layout-ncol=2}
![](../models/2023/DiscardVariance/bridging/3-compare1_spawnbio.png)

![](../models/2023/DiscardVariance/bridging/3-compare3_Bratio.png)
:::
...
deviations <- report[["parameters"]] |>
  ...
  tidyr::separate_wider_delim(
    cols = Label,
    delim = "_",
    names = c("Period", "Type", "Year")
  ) |>
  dplyr::select(Year, Deviation = Value)
deviations |>
  dplyr::slice_max(Deviation, n = 15) |>
  dplyr::mutate(
    Deviation = format(round(Deviation, 3), nsmall = 3)
  ) |>
  knitr::kable(align = c("r", "r"))
```
...
```{r}
deviations |>
  dplyr::slice_min(Deviation, n = 15) |>
  dplyr::mutate(
    Deviation = format(round(Deviation, 3), nsmall = 3)
  ) |>
  knitr::kable(align = c("r", "r"))
...
-    Sensitivities - Exploration
-    Sensitivities - Structural Changes
-    Sensitivities - Standard Set
...
## Retrospective - Deviations & WCGBTS

::: {layout-ncol=2}
![Log of recruitment deviations.](../models/2023/DiscardVariance/base_retro_5_yr_peel/compare12_recdevs_uncertainty.png)

![WCGBTS index.](../models/2023/DiscardVariance/base_retro_5_yr_peel/compare13_indices_flt7.png)
:::
...
-    To all that made this limited update happen within a very limited amount
     of time. Particularly the age readers at the Cooperative Ageing Program 
     and the staff in charge of the discard data within the West Coast
     Groundfish Observer Program.
-    To the members of the Population Ecology at the NWFSC for providing
     helpful tools that allow for rapid, reproducible research.
-    To Drs. Owen Hamel and Jim Hastie for reviewing the draft assessment.
