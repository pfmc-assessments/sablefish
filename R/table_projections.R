table_projection <- function(model_location,
                             model,
                             file_name = "projections.tex",
                             ...) {
  ...
  tab <- cbind(
    utils::read.csv(here::here("data-processed", "harvest_spex.csv")) |>
      dplyr::filter(
        AREA_NAME == "CW" & YEAR >= 2023 & PECIFICATION_TYPE == "OFL"
      ) |>
      dplyr::reframe(
  ...
  kableExtra::save_kable(
    t,
    file = fs::path(model_location, "tables", file_name)
  )
