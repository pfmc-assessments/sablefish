...
```{r tables-process-survey, eval = !fs::file_exists(fs::path(tables_location, "data-survey-n.tex"))}
process_survey()
```
...
recode_parameters <- function(x) {
  x |>
    gsub(pattern = "(.+)_(Fem)(.+)", replacement = "\\2ale \\1") |>
    gsub(pattern = "(.+)_(Mal)(.+)", replacement = "\\2e \\1") |>
...
\input{52tables/e_ReferencePoints.tex}