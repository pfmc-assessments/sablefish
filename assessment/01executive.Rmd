...
```{r, results = "asis"}
sa4ss::read_child(fs::path(model_location, "tables", "b_SSB_ES.tex"))
```
...
```{r results = "asis"}
sa4ss::read_child(fs::path(model_location, "tables", "c_Recr_ES.tex"))
```
...
```{r results = "asis"}
sa4ss::read_child(fs::path(model_location, "tables", "d_SPR_ES.tex"))
sa4ss::add_figure(
  filein = fs::path(
    model_location,
    "plots",
...
The estimated 2023 spawning biomass relative to unfished equilibrium spawning biomass is `r round(100*model[["current_depletion"]], 2)`\%, well above the management target of 40\% of unfished spawning output. The fishing intensity has been at or below the current management harvest rate limit (spawning potential ratio [SPR], $\text{SPR}_{45\%}$) since the mid 1980s and declining in the last ten years (Figure \@ref(fig:es-phase)).  The relative spawning output compared to the fishing intensity (SPR relative to the management target $\text{SPR}_{45\%}$) across all model years are shown in Figure \@ref(fig:es-phase) where warmer colors (red) represent early years and colder colors (blue) represent recent years. The relative biomass and estimated SPR has been well above the-es) shows the equilibrium curve based on a steepness value fixed at `r round(model$parameters[model$parameters$Label == "SR_BH_steep","Value"], 2)` with vertical dashed lines to indicate the estimate of fraction unfished at the start of `r endyr +1` (current) and the estimated management targets calculated based on the relative target biomass (B target), the SPR target, and the maximum sustainable yield (MSY).
