...
The prior distribution for natural mortality ($M$) by sex remains the same as what was assumed in the 2019 benchmark assessment and were based on the Hamel [-@hamel_method_2015] meta-analytic approach with  an assumed maximum age of 102 years for females and 98 for males. The prior assumed a log normal distribution for natural mortality with a median of 0.053 and 0.055 yr^-1^ and a standard error of 0.438. 

The base model fixed steepness ($h$) at 0.7. Like many assessments, this assessment is unable to estimate
...
recruitment), $M$, and $h$. Likelihood profiles for $h$ in past `r spp` assessments suggest that there is
...
reduced because both and $F$ and $SB_{proxy}$ are used for management rather than $MSY$


### Data Weighting

Length data from the \gls{s-wcgbt} and fishery discard lengths, conditional age-at-length compositions from the \gls{s-wcgbt}, and marginal age compositions from the fishery fleets and other surveys were fit and appropriately weighted within the model.  It was assumed for age-at-length data that each age was a random sample within the length bin and the model started with a sample size equal to the number of fish in that length bin. Extra variability parameters were estimated and added to the input variance for the indices of abundance for three surveys in the model; 1) \gls{s-tri} both early and late, 2) \gls{s-nwslope}, and 3) \gls{s-aslope} as well as the environmental sea-surface height index. Estimating additional variance for the \gls{s-wcgbt} was explored and determined to not be required. 

The base assessment model was weighted using the Francis method, which was based on equation TA1.8 in Francis [@francis_data_2011]. Sensitivities were performed examining the difference when using the McAllister Ianelli [@mcallister_bayesian_1997] method (Harmonic Mean weighting). The weights applied to each length and age data set for the base model are shown in Table XX under the Francis method column.


### Model Parameters

There were 235 estimated parameters in the base model. These included one parameter for $R_0$, 10 parameters for growth, 2 parameters for sex-specific natural mortality, 4 parameters for extra variability for the survey indices, 45 parameters for age-based selectivity, length-based retention, and time blocking of the fleets and the surveys, 163 recruitment deviations (including 30 pre-model estimates), and 10 forecast recruitment deviations which were fixed to 0 (Table XX).

Fixed parameters in the model were as follows: Steepness was fixed at 0.70. The standard deviation of recruitment deviates was fixed at 1.4 ($\sigma_R$). Maturity-at-length and length-weight parameters were fixed at the values used in the 2019 benchmark assessment. Dome-shaped selectivity was explored for all fleets within the model. Older `r spp` are often found in deeper waters and may move into areas that limit their availability to fishing gear. 
