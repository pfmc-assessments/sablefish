...
ignore <- fs::dir_ls(
  ...
  dplyr::mutate(
    label = paste(
      "bridging",
      gsub("(.+)_(.+)(\\.png)", "\\1-\\2", basename(filein)),
      sep = "-"
    ),
...
sa4ss::add_figure(
  filein = file.path(model_location, "plots", "comp_lenfit_flt7mkt0.png"), 
  caption = "Pearson residuals for \\glsentrylong{s-wcgbt} length data. Closed bubble are positive residuals (observed > expected) and open bubbles are negative residuals (observed < expected)",
  label = "comp-lenfit-flt7mkt0"
)
```

```{r figures-fit-caal, results = "asis"}
ignore <- fs::dir_ls(
  path = fs::path(model_location, "plots"),
  regex = "comp_condAALfit_Andre"
) |>
  tibble::as_tibble_col(column_name = "filein") |>
  dplyr::mutate(
    fleet_numeric = as.numeric(gsub(".+flt([0-9]+)mkt.+", "\\1", filein)),
    fleet = recode_fleet_text(fleet_numeric),
    caption = glue::glue("Year-specific conditional age-at-length data (left) and standard deviation (stdev) at age (right) from the {fleet}. Shaded areas are confidence intervals based on adding 1.64 standard errors of the mean to the mean age and 90 percent intervals from a chi-square distribution for the standard deviation of mean age."),
    label = gsub(pattern = "_", "-", gsub("\\.png", "", basename(filein)))
  ) |>
  dplyr::select(filein, caption, label) |>
  purrr::pmap(.f = sa4ss::add_figure)
...
```{r figures-fit-ages, results = "asis"}
sa4ss::add_figure(
...
sa4ss::add_figure(
  filein = fs::path(model_location, "plots",  "comp_lenfit_flt1mkt1_page1.png"), 
  caption = "Fits to the discard length composition by year for the fixed gear fleet.",
  label = "comp-disc-len-fixed-1"
)
sa4ss::add_figure(
  filein = fs::path(model_location, "plots",  "comp_lenfit_flt1mkt1_page2.png"), 
  caption = "Fits to the discard length composition by year for the fixed gear fleet.",
  label = "comp-disc-len-fixed-2"
)
sa4ss::add_figure(
  filein = fs::path(model_location, "plots",  "comp_lenfit_flt2mkt1_page1.png"), 
  caption = "Fits to the discard length composition by year for the trawl gear fleet.",
  label = "comp-disc-len-trawl-1"
)
sa4ss::add_figure(
  filein = fs::path(model_location, "plots",  "comp_lenfit_flt2mkt1_page2.png"), 
  caption = "Fits to the discard length composition by year for the trawl gear fleet.",
  label = "comp-disc-len-trawl-2"
)
ignore <- fs::dir_ls(
  path = fs::path(model_location, "plots"),
  regex = "agefit.+residsflt"
) |>
  tibble::as_tibble_col(column_name = "filein") |>
  dplyr::mutate(
    fleet_numeric = as.numeric(gsub(".+flt([0-9]+)mkt.+", "\\1", filein)),
    ...
    type = dplyr::case_when(
      grepl("mkt0", filein) ~ "",
      grepl("mkt1", filein) ~ "discarded",
      grepl("mkt2", filein) ~ "retained"
    ),
    caption = glue::glue("Pearson residuals for the fits to the {fleet} {type} age-composition data. Filled circles represent positive residuals (observed-expected) where red and blue are female and male, respectively."),
    label = gsub(pattern = "_", "-", gsub("\\.png", "", basename(filein)))
  ) |>
  dplyr::select(filein, caption, label) |>
  purrr::pmap(.f = sa4ss::add_figure)
...
ignore <- fs::dir_ls(
  ...
  tibble::as_tibble_col(column_name = "filein") |>
  dplyr::mutate(
    fleet = recode_fleet_text(as.numeric(recode_fleet(gsub(".+flt(.+)\\.png", "\\1", filein)))),
    caption = glue::glue("Fits to the {fleet} discard mean body weight data."),
    label = gsub(pattern = "_", "-", gsub("\\.png", "", basename(filein)))
  ) |>
  dplyr::select(filein, caption, label) |>
  purrr::pmap(.f = sa4ss::add_figure)
...
ignore <- fs::dir_ls(
  ...
  tibble::as_tibble_col(column_name = "filein") |>
  dplyr::mutate(
    label = paste("sensitivity", gsub("(.+)_(.+)(\\.png)", "\\1-\\2", basename(filein)), sep = "-"),
    type = dplyr::case_when(
      grepl("1|2", basename(filein)) ~ "Spawning output",
      grepl("3|4", basename(filein)) ~ "Fraction unfished"
    ),
