...
ignore <- tidyr::expand_grid(
  folder_list = fs::dir_ls(dirname(model_location), regexp = "base_retro"),
  string = c("compare2", "compare4")
) |>
  dplyr::mutate(
    ...
    label = paste("retro", string, sep = "-"),
    type = dplyr::case_when(
      grepl("2", string) ~ "spawning output",
      grepl("4", string) ~ "fraction unfished"
    ),
    caption = glue::glue(
      "Change in the estimate of {type} when the most recent 5 years of data are removed sequentially."
    )
  ) |>
