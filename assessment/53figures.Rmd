...
ignore <- fs::dir_ls(
  ...
  dplyr::mutate(
    ...
    type = dplyr::case_when(
      grepl("compare[1-2]_", basename(filein)) ~ "Spawning output",
...
ignore <- tidyr::expand_grid(
  ...
  string = c("compare1_", "compare3_", "piner")
) |>
  dplyr::mutate(
    ...
    type = dplyr::case_when(
      ...
      grepl("compare[1-2]_", string) ~ "spawning biomass",
      grepl("compare[3-4]_", string) ~ "fraction unfished"
    ),
    caption = glue::glue(
